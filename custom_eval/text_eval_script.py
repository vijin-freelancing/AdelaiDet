...
import sys
import numpy as np
from shapely.geometry import Polygon as plg, LinearRing
...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    print(f"number of files in groundtruth: {len(gt)}")
    
    subm = rrc_evaluation_funcs.load_zip_file(submFilePath, evaluationParams['DET_SAMPLE_NAME_2_ID'], True)
    print(f"number of files in submission: {len(subm)}")
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    print(f"groundtruth files: {len(gt)}, submission_files: {len(subm)}")
